...
extern const struct fimc_is_vender_rom_addr *vender_rom_addr[SENSOR_POSITION_END];
#endif
...
int sensor_gc5035_cis_check_rev(struct fimc_is_cis *cis)
{
	...
	fimc_is_sensor_addr8_write8(client, 0xfe,  0x02);
	probe_info("gc5035 sensor page selection write complete\n");

	/* Turn ON OTP Read MODE */
	fimc_is_sensor_addr8_write8(client, 0x67,  0xc0);
	probe_info("gc5035 sensor OTP/OTP clk enable complete\n");
	
	/* Turn ON OTP Read MODE */
	fimc_is_sensor_addr8_write8(client, 0x55,  0x80);
	probe_info("gc5035 sensor enable OTP read mode complete\n");
	
	/* Turn ON OTP Read MODE */
	fimc_is_sensor_addr8_write8(client, 0x66,  0x03);
	probe_info("gc5035 sensor enable OTP read mode complete\n");

	/* Turn ON OTP Read MODE */
	fimc_is_sensor_addr8_write8(client, 0xf3,  0x01);

	/* Check status - 0x01 : read ready*/
	fimc_is_sensor_addr8_read8(client, 0x6c,  &status);
	if ((status & 0x1) == false)
		err("status fail, (%d)", status);

	/* Module ID Read */
	fimc_is_sensor_read8(client, 0x1f18,  &rev);
	probe_info("gc5035 sensor read Module ID rev = %d\n", rev);

	/* Lens ID Read */
	fimc_is_sensor_read8(client, 0x1f20,  &rev);
	probe_info("gc5035 sensor read Lens ID rev = %d\n", rev);
...
int sensor_gc5035_cis_init(struct v4l2_subdev *subdev)
{
	...
	info("[%s] start\n", __func__);
#if 0
	ret = sensor_gc5035_cis_check_rev(cis);
	if (ret < 0) {
...
int sensor_gc5035_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	...
	if (ret < 0)
		goto p_err;
#if 0	
	ret = fimc_is_sensor_write8(client, 0x0901, (ratio_w << 4) | ratio_h);  //TODO:Binning type address
	if (ret < 0)
		goto p_err;
#endif
...
#if 0
	ret = fimc_is_sensor_write8(client, 0x0400, 0x0000);
	if (ret < 0)
		goto p_err;
	/* down_scale_m: 1 to 16 upwards (scale_n: 16(fixed))
	down scale factor = down_scale_m / down_scale_n */
	ret = fimc_is_sensor_write8(client, 0x0404, 0x0010);
	if (ret < 0)
		goto p_err;
#endif
	cis_data->frame_time = (cis_data->line_readOut_time * cis_data->cur_height / 1000);
