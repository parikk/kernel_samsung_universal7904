...
void decon_dump(struct decon_device *decon)
{
	...
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs, 0x2A8, false);

	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs + 0x0400, 0xA4, false);
	...
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs + SHADOW_OFFSET, 0x290, false);
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
...
static int decon_enable(struct decon_device *decon)
{
	...
	if (decon->dt.out_type == DECON_OUT_DP) {
		decon_wb_free_irq(decon);

		ret = decon_displayport_register_irq(decon);
		if (ret) {
			decon_err("decon_displayport_register_irq fail.\n");
			goto err;
		}
...
static int decon_disable(struct decon_device *decon)
{
	...
	if (decon->dt.out_type == DECON_OUT_DP) {
		...
		decon_displayport_free_irq(decon);
...
static void __decon_update_regs(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	if (decon->dt.out_type == DECON_OUT_WB) {
		decon_dbg("blender bg: w(%d), h(%d)\n",
				regs->blender_bg.right + 1,
				regs->blender_bg.bottom + 1);
		decon->lcd_info->xres = regs->blender_bg.right + 1;
		decon->lcd_info->yres = regs->blender_bg.bottom + 1;
		...
		regs->dpp_config[MAX_DECON_WIN].src.w = regs->blender_bg.right + 1;
		regs->dpp_config[MAX_DECON_WIN].src.h = regs->blender_bg.bottom + 1;
		regs->dpp_config[MAX_DECON_WIN].dst.w = regs->blender_bg.right + 1;
		regs->dpp_config[MAX_DECON_WIN].dst.h = regs->blender_bg.bottom + 1;
	}
			...
			unsigned long arg)
{
	...
	case S3CFB_FORCE_PANIC:
	        decon_info("Request force panic from platform\n");
		decon_dump(decon);
	        BUG();
	        break;
